...
The manpage section is `home-configuration.nix(5)`. If the option isn't there, it likely belongs to NixOS rather than home-manager — try `man configuration.nix` (NixOS) or `nixos-option <path>` instead.
